        ...
        if (tempParamKey === null) {
          ...
          if (change == '#') {
            ...
            if (isObject(tempParamKey) || isArray(tempParamKey)) {
              // 对象或数组直接 JSON 序列化，不需要额外转义
              tempParamKey = JSON.stringify(tempParamKey);
            } else {
              // 转为字符串
              var stringValue = tempParamKey.toString();
              
              // 判断是否为 JSON 格式（以 { 或 [ 开头）
              var isJsonString = /^\s*[\{\[]/.test(stringValue);
              ...
              tempParamKey = mysqlRealEscapeParam(stringValue);
            }
